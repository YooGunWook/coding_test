...
# dp 기반 풀이
def solution(item, n):
    item[0][1] = item[0][1] + item[1][0] # 1번째 인덱스에 전에 대각선 값 넣기
    item[1][1] = item[1][1] + item[0][0]
    for i in range(2, n): # 대각선에 있는 i-1, i-2 중에 큰 값이랑 더해주기
        item[0][i] = max(item[1][i - 1], item[1][i - 2]) + item[0][i]
        item[1][i] = max(item[0][i - 1], item[0][i - 2]) + item[1][i]

    return max(item[0][n - 1], item[1][n - 1]) # 최종값 선정
...
for _ in range(t):
    ...
    for _ in range(2):
        ...
        full_item.append(sub_item)
    if n == 1: # n이 1이면 바로 계산 가능
        print(max(full_item[0][0], full_item[1][0]))
    else: # 그 외
        print(solution(full_item, n))
