...
# DP로 문제 풀기
def solution(wine, n):
    dp = [0] * n  # 마신양을 체크하기 위한 List
    # 마시는 잔의 개수가 1~3개 사이면 dp없이 바로 결과 만들 수 있음
    if n == 1:
        return wine[-1]
    if n == 2:
        return sum(wine)
    ...
    else:  # 3개보다 많이 마시게 되면 dp를 통해 계산해야한다.
        dp[0] = wine[0]
        dp[1] = sum(wine[:2])
        dp[2] = max(dp[1], wine[0] + wine[2], wine[1] + wine[2])
        for i in range(3, n):
            dp[i] = max(
                wine[i] + wine[i - 1] + dp[i - 3], wine[i] + dp[i - 2], dp[i - 1]
            )  # max(현재 마셔야되는 와인  + 이전에 마신 와인 + 현재 마셔야되는 와인 - i-3번째에서 최대로 마신 와인,
            #      현재 마셔야되는 와인 + i-2번째에서 최대로 마신 와인, i-1번째에서 마신 최대 와인)
    return max(dp)
